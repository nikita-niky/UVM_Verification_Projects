...
class fsm_test extends uvm_test;
  ...
  function new(string name = "fsm_test", uvm_component parent);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    seq = fsm_master_seq::type_id::create("seq");
  ...
  virtual function void report_phase(uvm_phase phase);
    ...
    if (svr.get_severity_count(UVM_ERROR) == 0) begin
      `uvm_info("TEST_RESULT", "--- PASSED ---", UVM_NONE)
    end else begin
      `uvm_error("TEST_RESULT", "--- FAILED ---")
    end
  endfunction
