...
class fsm_scoreboard extends uvm_scoreboard;
  ...
  virtual function void write(fsm_item tr);
    ...
    if(tr.rst_n && exp_state !== 2'bxx) begin
    ...
    if(tr.rst_n) begin
      if(exp_state==S101 && tr.bit_in == 1) begin ///comparing the output 
        if(tr.pattern_found !==1'b1)
          `uvm_error("OUT_ERR","Pattern found should be HIGH in S101")        
          end
          else begin 
    ...
    if(!tr.rst_n) begin
      ...
      case(exp_state)  ///updating reference model for next clock
        ...
        S101:  exp_state = tr.bit_in ? S1: S10; ///overlapping 
